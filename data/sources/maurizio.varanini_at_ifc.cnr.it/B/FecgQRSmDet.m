function qrsRef=FecgQRSmDet(Se,fs,cName,graph,dbFlag,saveFig,qrsAf)
% ---------------------------------------------------------------------------------------------
...
if(dbFlag && learning), PlotSgnMrkNc(Se, qrsAf*fs, fs, cName); end
...
% raw derivative filter coefficients (7ms before and after, 9ms in the middle)
...
pth=0.45; qrsRef=QRSdetectorM(adecg1x,decg1x,fs,pth,0.85,1);
...
if(graph && dbFlag)
    ...
    DrawVertMarker(qrsRef','r',':','none');           % QRS reference point
    subplot(2,1,2),plot(ecg1,'b');
    DrawVertMarker(qrsRef(1,:)','r',':','none');      % QRS reference point
end

if(graph || saveFig)
    figure, set(gcf,'Color','white'); hold on;
    if(learning), plot(qrsAf, [RRafs(1);RRafs],'r.-'); end
    plot(qrsRef/fs, [RRs(1);RRs],'b.-');
    ylim([0.3, 2]);
...
if(graph)
    ...
    for is=1:ns,
        ...
        wgma1= max(Ses(:,is)) +2;
        ylim([wgmi1, wgma1]);
        DrawVertMarker(qrsRef'/fs,'r',':','none');
        set(gca,'YTick',[-5 0 5])
        % if(is~=ns), set(gca,'XTickLabel',''); end
...
if(graph || saveFig)
    PlotSgnMrkNc(ecgs(:,1), qrsRef, fs, cName);
    shg
    if(saveFig), figFmt='png';
        figPath=fullfile('../Figure/',progname);
        if(~exist(figPath,'dir')), mkdir(figPath); end
